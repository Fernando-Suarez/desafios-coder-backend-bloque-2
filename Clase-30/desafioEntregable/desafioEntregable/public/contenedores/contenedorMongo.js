const mongoDB = require('../options/configMongoDB');
const { modeloMensaje } = require('../models/modeloMongoMensajes');

class Contenedor {
	constructor(mongoDB, modeloMensaje) {
		this.mongoDB = mongoDB;
		this.modelo = modeloMensaje;
	}
	...
	async getById(id) {
		try {
			const productoId = await this.modelo.findOne({ id: id });
			if (!productoId) {
				return null;
			} else {
				return productoId;
